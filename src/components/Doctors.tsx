...
import { Phone, Clock, Award, Shield, ArrowUpRight } from "lucide-react";
  ...
  return (
    <section id="doctors" className="py-24 bg-brand-blush/20 border-b border-brand-border/40 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
        ...
        {/* Doctors Grid — show first 2 on homepage */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {doctors.slice(0, 2).map((doc: any, idx: number) => (
            <motion.div
              key={doc._id}
        ...
        {doctors.length > 2 && (
          <div className="flex justify-center mt-12">
            ...
            >
              <span>{lang === "en" ? "See All Doctors" : "அனைத்து மருத்துவர்கள்"}</span>
        ...
        <div className="flex gap-8 whitespace-nowrap animate-marquee-reverse">
          {[...Array(4)].map((_, idx) => (
            <div key={idx} className="flex gap-8 text-[10px] font-bold text-pink-safe uppercase tracking-widest">
              <span>✦ Manual Therapy Experts</span>
